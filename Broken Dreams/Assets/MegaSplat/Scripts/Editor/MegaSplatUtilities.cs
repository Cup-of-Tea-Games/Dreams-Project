...
namespace JBooth.MegaSplat
{
   ...
   public class MegaSplatUtilities 
   {
      ...
      static void RenderMeshPreview (Material mat) 
      {
         ...
         sPru.camera.backgroundColor = Color.gray;
         sPru.camera.clearFlags = CameraClearFlags.Color;
         sPru.camera.orthographic = true;
         sPru.camera.nearClipPlane = 0.3f;
         sPru.camera.farClipPlane = 40;
         sPru.camera.rect = new Rect(0, 0, 1, 1);
         sPru.camera.orthographicSize = 0.51f;
         sPru.camera.transform.position = new Vector3(0, 0, -2);
         sPru.camera.transform.LookAt(Vector3.zero);
         sPru.lights[0].transform.rotation = Quaternion.Euler(45f,45f,0f);
