...
//   Compiled with MegaSplat 1.14
//   Unity : 5.4.0f3
//   Platform : OSXEditor
//////////////////////////////////////////////////////

Shader "MegaSplat/BrushPreview_Standard" {
   Properties {
      // Splats
   ...
   SubShader {
      ...
      #define _TWOLAYER 1
         ...
         void SamplePerTex(sampler2D pt, inout LayerParams params, float2 scaleRange)
         {
            ...
            #if _PERTEXMATPARAMS || _PERTEXUV
            {
               ...
               params.uv2.xy *= uvScale.z;
                  ...
                  params.dxtpcoords.xy = ddx(params.tpuv0_x.xy);
                  params.dytpcoords.xy = ddy(params.tpuv0_x.xy);
                  #endif
         ...
         #if _RAINDROPS
         float3 ComputeRipple(float2 uv, float2 dx, float2 dy, float time, float weight)
         {
            float4 ripple = tex2Dgrad(_RainDropTexture, uv, dx, dy);
         ...
         float DoPuddles(inout MegaSplatLayer o, float2 uv, float2 dx, float2 dy, half3 waterNormFoam, half puddleLevel, half2 flowDir, half porosity, float3 worldNormal)
         {
            #if _PUDDLES || _PUDDLEFLOW || _PUDDLEREFRACT
            puddleLevel *= _MaxPuddles;
            ...
            half3 waterNorm = half3(0,0,1);
               ...
               half foam0 = tex2Dgrad(_PuddleNormal, foamUV0, dx, dy).b;
               half foam1 = tex2Dgrad(_PuddleNormal, foamUV1, dx, dy).b;
            ...
            WaterBRDF(wetAlbedo, wetSmoothness, o.Metallic, waterBlend, porosity);
               ...
               float3 ripple1 = ComputeRipple(ruv1 + float2( 0.25f,0.0f), dx, dy, times.x, weights.x);
               float3 ripple2 = ComputeRipple(ruv2 + float2(-0.55f,0.3f), dx, dy, times.y, weights.y);
               float3 ripple3 = ComputeRipple(ruv1 + float2(0.6f, 0.85f), dx, dy, times.z, weights.z);
               float3 ripple4 = ComputeRipple(ruv2 + float2(0.5f,-0.75f), dx, dy, times.w, weights.w);
         ...
         float DoLava(inout MegaSplatLayer o, float2 uv, float2 dx, float2 dy, half lavaLevel, half2 flowDir)
         {
            #if _LAVA
            ...
            //half4 lavaDist = tex2Dgrad(_LavaDiffuse, uv * distortionScale + _Time.y * distortionRate, dx, dy);
         ...
         #if _PUDDLEFLOW || _PUDDLEREFRACT
         half3 GetWaterNormal(float2 uv, float2 dx, float2 dy, float2 flowDir)
         {
            float2 uv1;
            float2 uv2;
            half interp;
            ...
            half3 fd = lerp(tex2Dgrad(_PuddleNormal, uv1, dx, dy), tex2Dgrad(_PuddleNormal, uv2, dx, dy), interp).xyz;
            fd.xy = fd.xy * 2 - 1;
            return fd;
         }
         #endif
         ...
         { 
            ...
            fixed4 albedo = tex0 * weights.x + tex1 * weights.y + tex2 * weights.z;
               ...
               bool resample = false;
               #if _PUDDLEREFRACT
               //float puddlesUV = si.macroUV * _PuddleUVScales.xy;
               //float puddlesDX = ddx(puddlesUV);
               //float puddlesDY = ddy(puddlesUV);
         ...
         float DoSnow(inout MegaSplatLayer o, float2 uv, float3 worldNormal, half snowHeightFade, float puddleHeight, half surfPorosity, float camDist)
         {
            ...
            float2 snowResampleDX = ddx(snowResampleUV);
            float2 snowResampleDY = ddx(snowResampleUV);
            UNITY_BRANCH
            if (camDist > _SnowDistanceResampleScaleStrengthFade.z)
               {
                  half4 snowAlb2 = tex2Dgrad(_SnowDiff, snowResampleUV, snowResampleDX, snowResampleDY);
                  half4 snowNsao2 = tex2Dgrad(_SnowNormal, snowResampleUV, snowResampleDX, snowResampleDY);
               ...
               float2 snowNoiseDX = ddx(snowNoiseUV);
               float2 snowNoiseDY = ddx(snowNoiseUV);
         ...
         void ApplyDetailNoise(inout half3 albedo, inout half3 norm, inout half smoothness, in LayerParams data, float camDist, float3 tpw)
         {
            ...
            #if _DISTANCENOISE
            {
               ...
               {
                  #if _TRIPLANAR
                  half3 noise = (tex2Dgrad(_DistanceNoise, uv0, dx0, dy0) * tpw.x + 
                     tex2Dgrad(_DistanceNoise, uv1, dx1, dy1) * tpw.y + 
                     tex2Dgrad(_DistanceNoise, uv2, dx2, dy2) * tpw.z).rgb; 
                  #else
                  half3 noise = tex2Dgrad(_DistanceNoise, uv, dx, dy).rgb;
                  #endif
         ...
         MegaSplatLayer DoSurf(inout SplatInput si, MegaSplatLayer macro, float3x3 tangentToWorld)
         {
            ...
            float2 puddleDX = ddx(puddleUV);
            float2 puddleDY = ddy(puddleUV);
            #elif _PUDDLES
            float2 puddleUV = 0;
            float2 puddleDX = 0;
            float2 puddleDY = 0;
            ...
            if (si.puddleHeight > 0 && si.camDist.y < 40)
            {
               ...
               si.waterNormalFoam = GetWaterNormal(puddleUV, puddleDX, puddleDY, si.flowDir) * str;
            }
            #endif
            ...
            float pud = 0;
               ...
               {
                  pud = DoPuddles(splats, puddleUV, puddleDX, puddleDY, si.waterNormalFoam, si.puddleHeight, si.flowDir, porosity, worldNormal);
               }
            #endif

            #if _LAVA
               float2 lavaUV = si.macroUV * _LavaUVScale.xy;
               float2 lavaDX = ddx(lavaUV);
               float2 lavaDY = ddy(lavaUV);

               UNITY_BRANCH
               if (si.puddleHeight > 0)
               {
                  pud = DoLava(splats, lavaUV, lavaDX, lavaDY, si.puddleHeight, si.flowDir);
               }
            #endif
