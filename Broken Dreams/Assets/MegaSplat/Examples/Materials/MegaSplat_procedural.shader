...
Shader "MegaSplat/MegaSplat_procedural" {
   Properties {
      // Splats
      ...
      // auto paint
      _ProjectTexTop("Top Textures", Vector) = (0,0,0)
      _ProjectTexSide("Side Textures", Vector) = (0,0,0)
      _ProjectTexBottom("Bottom Textures", Vector) = (0,0,0)
      _ProjectTexThresholdFreq("bottom/top Threshold and frequency of cluster noise", Vector) = (-0.3333, 0.3333, 0.05)
      // auto paint
      _ProjectTexTop2("Top Textures", Vector) = (0,0,0,0)
      _ProjectTexSide2("Side Textures", Vector) = (0,0,0,0)
      _ProjectTexBottom2("Bottom Textures", Vector) = (0,0,0,0)
      _ProjectTexThresholdFreq2("bottom/top Threshold and frequency of cluster noise", Vector) = (-0.3333, 0.3333, 0.05)
      _ProjectTexBlendParams("Noise freq, amp, min, max", Vector) = (0.05, 1.0, 0, 1)
   }
   SubShader {
      ...
      #define _NORMALMAP 1
      #define _PROJECTTEXTURE2_WORLD 1
      #define _PROJECTTEXTURE_WORLD 1
      #define _TWOLAYER 1
         ...
         float FlowRefract(MegaSplatLayer tex, inout LayerParams main, inout LayerParams second, half3 weights)
         {
            ...
            return 1;
               ...
               #define SAMPLETEXARRAY(t0, t1, t2, TA, lp) \
                  t0  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv0_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv0_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv0_z); \
                  t1  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv1_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv1_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv1_z); \
                  t2  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv2_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv2_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.tpuv2_z); \
                  { \
                     half4 st0  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv0_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv0_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv0_z); \
                     half4 st1  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv1_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv1_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv1_z); \
                     half4 st2  = tpw.x * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv2_x) + tpw.y * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv2_y) + tpw.z * UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_tpuv2_z); \
                  ...
                  if (lp.flowOn > 0) \
                  { \
                     t0 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv0a), UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv0b), lp.flowInterps.x); \
                     t1 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv1a), UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv1b), lp.flowInterps.y); \
                     t2 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv2a), UNITY_SAMPLE_TEX2DARRAY(TA, lp.fuv2b), lp.flowInterps.z); \
                  } \
                  else \
                  { \
                     t0 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv0), UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_uv0), lp.distanceBlend); \
                     t1 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv1), UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_uv1), lp.distanceBlend); \
                     t2 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv2), UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_uv2), lp.distanceBlend); \
                  }
               #else
                  #define SAMPLETEXARRAY(t0, t1, t2, TA, lp) \
                     t0 = lerp(UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv0), UNITY_SAMPLE_TEX2DARRAY(TA, lp.db_uv0), lp.distanceBlend); \
               ...
               #define SAMPLETEXARRAY(t0, t1, t2, TA, lp) \
                  ...
                  { \
                     t0 = UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv0); \
                     t1 = UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv1); \
                     t2 = UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv2); \
                  }
               #else
                  #define SAMPLETEXARRAY(t0, t1, t2, TA, lp) \
                     t0 = UNITY_SAMPLE_TEX2DARRAY(TA, lp.uv0); \
      ...
      void vert (inout appdata_full i, out Input o) 
      {
          ...
          worldNormal = normalize(mul(unity_ObjectToWorld, i.normal));
          #endif
