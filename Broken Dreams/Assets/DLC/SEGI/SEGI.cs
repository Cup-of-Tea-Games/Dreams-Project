...
public class SEGI : MonoBehaviour
{
	...
	public bool aggressiveTemporalSampling = true;
	[Range(0.01f, 1.0f)]
	...
	public float vramUsage
	{
		get
		{
			...
			if (rg0 != null)
				v += rg0.width * rg0.height * rg0.volumeDepth * 32;

			if (ba0 != null)
				v += ba0.width * ba0.height * ba0.volumeDepth * 32;
	...
	public void ApplyPreset(SEGIPreset preset)
	{
		...
		aggressiveTemporalSampling = preset.aggressiveTemporalSampling;
	...
	void CreateVolumeTextures()
	{
		...
		if (rg0)
		{
			rg0.DiscardContents();
			rg0.Release();
			DestroyImmediate(rg0);
		}
		rg0 = new RenderTexture((int)voxelResolution, (int)voxelResolution, 0, RenderTextureFormat.RInt, RenderTextureReadWrite.Linear);
		rg0.isVolume = true;
		rg0.volumeDepth = (int)voxelResolution;
		rg0.enableRandomWrite = true;
		rg0.filterMode = FilterMode.Point;
		rg0.Create();
		rg0.hideFlags = HideFlags.HideAndDontSave;

		if (ba0)
		{
			ba0.DiscardContents();
			ba0.Release();
			DestroyImmediate(ba0);
		}
		ba0 = new RenderTexture((int)voxelResolution, (int)voxelResolution, 0, RenderTextureFormat.RInt, RenderTextureReadWrite.Linear);
		ba0.isVolume = true;
		ba0.volumeDepth = (int)voxelResolution;
		ba0.enableRandomWrite = true;
		ba0.filterMode = FilterMode.Point;
		ba0.Create();
		ba0.hideFlags = HideFlags.HideAndDontSave;
	...
	void CleanupTextures()
	{
		...
		CleanupTexture(ref rg0);
		CleanupTexture(ref ba0);
		for (int i = 0; i < volumeTextures.Length; i++)
		{
	...
	void OnPreRender()
	{
		...
		if (renderState == RenderState.Voxelize)
		{
			...
			Shader.SetGlobalMatrix("SEGIVoxelViewFront", voxelCamera.transform.worldToLocalMatrix);
			Shader.SetGlobalMatrix("SEGIVoxelViewLeft", leftViewPoint.transform.worldToLocalMatrix);
			Shader.SetGlobalMatrix("SEGIVoxelViewTop", topViewPoint.transform.worldToLocalMatrix);
			...
			clearCompute.SetTexture(0, "RG0", rg0);
			...
			Graphics.SetRandomWriteTarget(1, rg0);
			Graphics.SetRandomWriteTarget(2, ba0);
			...
			transferInts.SetTexture(0, "RG0", rg0);
			transferInts.SetTexture(0, "BA0", ba0);
		...
		else if (renderState == RenderState.Bounce)
		{
			...
			transferInts.SetTexture(1, "RG0", rg0);
			transferInts.SetTexture(1, "BA0", ba0);
			transferInts.SetInt("Resolution", (int)voxelResolution);
	...
	void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		material.SetFloat("BlendWeight", aggressiveTemporalSampling ? temporalBlendWeight : 0.5f);
		...
		if (giRenderRes == 2)
		{
			...
			Graphics.Blit(aggressiveTemporalSampling ? gi3 : gi4, previousResult);
		...
		else
		{
			...
			Graphics.Blit(aggressiveTemporalSampling ? gi1 : gi2, previousResult);
			Graphics.Blit(source, previousDepth, material, Pass.GetCameraDepthTexture);

			material.SetTexture("GITexture", gi1);
			Graphics.Blit(source, destination, material, visualizeGI ? Pass.VisualizeGI : Pass.BlendWithScene);
		...
		frameSwitch = (frameSwitch + 1) % (aggressiveTemporalSampling ? 128 : 2);
	}
}
